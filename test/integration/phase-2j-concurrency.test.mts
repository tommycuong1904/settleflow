...
async function cleanup(f: Awaited<ReturnType<typeof fixture>>){await db.milestoneReview.deleteMany({where:{milestoneId:f.m.id}}); await db.milestoneSubmission.deleteMany({where:{milestoneId:f.m.id}}); await db.milestone.delete({where:{id:f.m.id}}); await db.payout.delete({where:{id:f.p.id}}); await db.contributor.delete({where:{id:f.c.id}}); await db.workspaceMember.deleteMany({where:{workspaceId:f.ws.id}}); await db.workspace.delete({where:{id:f.ws.id}}); await db.user.delete({where:{id:f.user.id}});}
